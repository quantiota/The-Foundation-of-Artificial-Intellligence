...
$\mathbf{z}^{(l)}_{k}$ is the knowledge vector and $\Delta \mathbf{D}^{(l)}_{k}$ is the shift in decision probabilities at step $k$. Unlike the static Shannon form, this entropy evolves continuously as knowledge accumulates, providing the local signal that drives forward-only, self-organized uncertainty reduction.
